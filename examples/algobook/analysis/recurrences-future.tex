...
%% reference to itself.
%% %
%% They commonly arise when analyzing the cost of recursive algorithms and
%% reflect the recursive structure of the algorithm.
...
%% \begin{example}[Mergesort Recurrence]
%% \label{ex:recurrences::mergesortold}
...
%% details.  
%% %
%% One such detail that is the base cases of the recurrence, which
%% usually specify the behavior of the algorithm on constant size inputs.
%% %
%% When the input size for a base case is constant, it usually adds an
%% asymptotically constant amount to the total cost.
%% %
%% Because we don't care to account for constant additive costs in
%% asymptotic notation, we usually omit such base cases.
 
%% %
%% For example, when stating the recurrence for merge sort, we ignored
%% the base cases and stated only the recursive case.  A more precise
%% statement of the recurrence would be
%% \[
%% W(n) = \left\{
%% \begin{array}{lll}
%% O(1) & \mbox{if} & n \le 1
...
%% \right.
%% \]
%% Because the base case only applies when $n \le 1$, it only adds a
%% constant to the $W(n)$ and  is thus omitted.
...
%% \begin{note}
%% An algorithm might have multiple cases some of which might apply to
%% inputs of non-constant size.  When writing the recurrence, it can
%% therefore be important to differentiate constants from non-constants.
%% \end{note}
...
%% The second technical issue concerns rounding of input sizes.
%% %
%% Going back to the \exref{recurrences::mergesortold}, note that the size
%% of the input to merge sort, $n$, is a natural numbers, but the
%% recurrence is stated for arguments such as $n/2$, which are not always
%% natural numbers.
%% %
%% In fact, the recursion that we stated for merge sort applies only to
%% input sizes that are powers of~$2$.  A more precise statement of the
...
%% When stating recurrences, we may use asymptotic notation to express
%% certain terms, e.g.  $O(n)$ in \exref{recurrences::mergesort}.  
%% %
%% Some care is needed when calculating the closed for a recurrence that
%% involves asymptotic terms, because we might accidentally add up
%% non-constant number of asymptotic terms. 
%% %
%% The trouble is that the sum of two $O(n)$ terms yield $O(n)$, but we
%% cannot repeat such a sum a non-constant number of times and still have
%% the result be $O(n)$.
%% %

%% To prevent mistakes in calculations, we therefore substitute such
%% terms with a non-asymptotic term.  For example, we may substitute
%% $O(n)$ with $n$, $2n$, $2n + \lg{n} + 3$, $3n + 5$, or with something
%% parametric such as $c_1n + c_2$ where $c_1$ and $c_2$ are constants.
%% %
%% These sorts of substitutions may introduce imprecision to our
%% calculations, because of the constant factors chosen.
%% %
%% This usually don't matter, however, because they change the closed-form
...
\subsection{A Formal Inductive Proof}
