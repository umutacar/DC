...
def find_embedding_index(embedding_sentence, keyphrase):
	...
	for i in range(len(embedding_sentence) - length_of_keyphrase + 2):
		if (embedding_sentence[i:i+length_of_keyphrase] == keyphrase_split):
			# if (keyphrase == 'turing machine'):
			# 	print(embedding_sentence[i:i+length_of_keyphrase])
			return i
	return -1
...
def create_definition_embeddings(definition_dict):
	...
	# for i in range(len(keyphrases)):
		# if (keyphrases[i] == 'Turing machine'):
		# 	print(definitions_clean[i])
	...
	for i in range(len(keyphrases)):
		...
		if (back_up_index == -1):
			...
			if (keyphrase == "the starting state of the TM"):
				end_index += 1
				begin_index = end_index - 20
			...
			if (keyphrase == 'the starting state of the TM'):
				print("hello ", embedding_sentence[index : index + length_of_keyphrase])

			print(index, length_of_keyphrase)
...
def filter_false_positives(definition_embedding, atoms, json_data):
	results = []
	for i in range(len(atoms)): # json_dict : dict(keyphrase) => list of indices
...
def filter_false_positives_per_atom(atom, json_dict, definition_embeddings):
	...
	for (keyphrase, indices) in json_dict.items():
		...
		for index_pair in indices:
			...
			# print(sentence_embedding[phrase_index: phrase_index + num_of_words])
			avg_phrase_embedding = np.mean(phrase_embedding, axis=0)
			avg_phrase_embedding /= np.linalg.norm(avg_phrase_embedding)
			print("definition embedding", definition_embedding)
			print('avg_phrase_embedding', avg_phrase_embedding)
...
if __name__=='__main__':
	...
	# print("wtf", definition_sentences['Turing machine'])
	embedding_definitions = create_definition_embeddings(definition_sentences)
	# print(embedding_definitions['the starting state of the TM'])
