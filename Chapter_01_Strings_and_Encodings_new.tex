%./texmlt -meta ./meta -preamble latex_preamble/preamble.tex ./01_\iref{definition:String-word-empty-string}{string}s_and_\iref{definition:Encoding-of-a-set}{encoding}s/Chapter_01_Strings_and_\iref{definition:Encoding-of-a-set}{encoding}s.tex
...
We often use the \iref{definition:String-word-empty-string}{word}s ``\iref{definition:String-word-empty-string}{string}'' and ``\iref{definition:String-word-empty-string}{word}'' to refer to a finite-length \iref{definition:String-word-empty-string}{string}/word. 
...
The \iref{definition:String-word-empty-string}{string} $\s{101}$ is a \iref{definition:Substring}{substring} of $\s{11011}$ and also a \iref{definition:Substring}{substring} of $\s{0101}$. 
On the other hand, it is not a \iref{definition:Substring}{substring} of $\s{1001}$.
\end{example}
\end{flex}
...
The \iref{definition:Concatenation-of-languages}{concatenation of languages} $\{\epsilon, \s{1}\}$ and $\{\s{0}, \s{01}\}$ is the \iref{definition:Language}{language}
\[
    \{\s{0}, \s{01}, \s{10}, \s{101}\}.
\] 
\end{example}
...
If $w \in \Sigma^*$ is a \iref{definition:String-word-empty-string}{word} that corresponds to a \iref{definition:Encoding-of-a-set}{valid encoding} of a pair of numbers $(x, y)$ (i.e., $\text{Enc}(x,y) = w$), then define $f(w)$ to be $\text{Enc}'(x+y)$. 
If $w \in \Sigma^*$ is \emph{not} a \iref{definition:String-word-empty-string}{word} that corresponds to a \iref{definition:Encoding-of-a-set}{valid encoding} of a pair of numbers (i.e., $w$ is not in the image of $\text{Enc}$), then define $f(w)$ to be $\s{\$}$. 
...
If $w \in \Sigma^*$ is a \iref{definition:String-word-empty-string}{word} that corresponds to an \iref{definition:Encoding-of-a-set}{encoding} of a prime number, then define $f(w)$ to be $\text{True}$. Otherwise, define $f(w)$ to be $\text{False}$. (Note that in the case of $f(w) = \text{False}$, either $w$ is the \iref{definition:Encoding-of-a-set}{encoding} of a composite number, or $w$ is not a \iref{definition:Encoding-of-a-set}{valid encoding} of a natural number.
\end{example}
\end{flex}
